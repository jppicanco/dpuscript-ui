...
from config import (
    ARQUIVADOS_DIR,
    DPU_WORKSPACE as OFICIO_GERAL,
    ENTRADA_DIR,
)
from services.skills_catalog import listar_skills
...
def _pasta_paj_existe(paj_norm: str) -> Path | None:
    """Retorna a pasta do PAJ (ativo ou arquivado) ou None."""
...
# Formato JP de paj_norm: "2026-039-08248" (4 digitos ano, 3 digitos forum, 5 digitos seq)
_PAJ_NORM_RE = re.compile(r"^\d{4}-\d{3}-\d{5,6}$")


# ---------------------------------------------------------------------------
# Persistencia em disco (em <PAJ>/conversas/<id>.json)
# ---------------------------------------------------------------------------

def _validar_paj_norm(paj_norm: str) -> bool:
    """Aceita o formato canonico JP: YYYY-NNN-NNNNN(N). Bloqueia path traversal."""
    if not paj_norm or "/" in paj_norm or "\\" in paj_norm or ".." in paj_norm:
        return False
    if not _PAJ_NORM_RE.match(paj_norm):
        return False
    return _pasta_paj_existe(paj_norm) is not None
...
def _pasta_conversas(paj_norm: str) -> Path | None:
    if not _validar_paj_norm(paj_norm):
        return None
    pasta = _pasta_paj_existe(paj_norm)
    return pasta / "conversas" if pasta else None
...
def _localizar_conversa(conv_id: str) -> tuple[str, Path] | None:
    ...
    for base in (ENTRADA_DIR, ARQUIVADOS_DIR):
        if not base.exists():
            continue
        for paj_dir in base.iterdir():
            if not paj_dir.is_dir() or not _PAJ_NORM_RE.match(paj_dir.name):
                continue
...
def _ler_prompt_max(paj_norm: str) -> str:
    """Le o PROMPT_MAX.md do PAJ (se existir)."""
    pasta = _pasta_paj_existe(paj_norm)
    if pasta is None:
        return ""
...
def _montar_prompt_inicial(paj_norm: str, skill_slug: str | None) -> str:
    ...
    return (
        ...
        "## ARQUITETURA — quem decide e quem corrige (LEIA)\n"
        "Os despachos e decisoes deste sistema sao feitos automaticamente pelo "
        "**Grok** (rodando neste M4, varias vezes ao dia). VOCE (Claude) e o "
        "**revisor/professor do Grok**. Quando o Defensor apontar um ERRO num "
        "despacho/decisao do Grok, sua funcao e:\n"
        ...
        "`/Users/macmini/dpu-workspace/skills/ensinar-grok/SKILL.md` (formato: "
        "ERRO COMUM / REALIDADE / REGRA / Exemplo / Origem: JP correcao em <data>). "
        "Acrescente 1 linha em `/Users/macmini/dpu-workspace/dpuscript/memory/correcoes_grok.log`.\n"
        "3. Oferecer corrigir o despacho DESTE PAJ tambem, se o Defensor quiser.\n"
        "IMPORTANTE: o Grok so aprende pelo que voce ESCREVER nessas regras. "
        "Ao concordar com uma correcao, **escreva a regra** — nao apenas concorde. "
        "Edite os arquivos por caminho absoluto (este chat roda no M4).\n"
        f"{skill_bloco}\n"
